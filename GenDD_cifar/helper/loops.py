...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            mask = torch.argmax(logit_t, dim=-1) == target

        diff_loss, output = model_s(input, target, feat_t[-1])
        ...
        max_norm=1.0
        total_norm = torch.nn.utils.clip_grad_norm_(model_s.parameters(), max_norm)
        #print(f"Total gradient norm (after clipping): {total_norm:.4f}, {diff_loss: .4f}")
        #if total_norm >100.0:
        #    continue
