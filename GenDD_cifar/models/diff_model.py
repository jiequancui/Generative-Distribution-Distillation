...
class DiffModel(nn.Module):
    def __init__(self, model, classifier=None, finetune_classifier=False, feature_dim=1024, target_dim=1024, diffloss_d=3, diffloss_w=1024, num_sampling_steps='100', diffusion_batch_mul=50, grad_checkpointing=False, stage="stage-1", smooth=0.9):
        ...
        if stage == 'stage-2':
            ...
            cls_weight = F.softmax(self.classifier.weight, dim=0) # 100x256
    ...
    def forward_train_stage2(self, x, y, target_feature):
        ...
        mix_target_feature = mix_target_feature.reshape(bsz, -1).repeat(self.diffusion_batch_mul, 1)
        mix_z = mix_z.reshape(bsz, -1).repeat(self.diffusion_batch_mul, 1)
        weight = weight.reshape(bsz, -1).repeat(self.diffusion_batch_mul, 1)
    ...
    def forward_test_stage2(self, x):
        ...
        #self.diffloss.num_timesteps = self.num_timesteps[0]
        sample_features = self.diffloss.sample(z, temperature=1.0)
        logits = self.classifier(sample_features)
        # multiple sampling
        #for i in range(9):
        #    #self.diffloss.num_timesteps = self.num_timesteps[i+1]
        #    sample_features = self.diffloss.sample(z, temperature=1.0)
        #    logits += self.classifier(sample_features)
        #logits = logits / 10.0
        return [logits, logits_new]

    def forward(self, x, y=None, target_feature=None):
