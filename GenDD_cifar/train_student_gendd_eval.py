...
from models.diff_model_eval import DiffModel
...
def param_groups_setup(model, opt, skip_list=()):
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue  
        # group setup
        if 'ln' in name or 'diffloss' in name:
...
def main():
    ...
    optimizer = torch.optim.AdamW(param_groups, betas=(0.9, opt.adam_beta_min), lr=opt.learning_rate)
