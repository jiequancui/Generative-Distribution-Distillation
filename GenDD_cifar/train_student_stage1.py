...
def parse_option():
    ...
    parser.add_argument('--min_learning_rate', type=float, default=0.0, help='learning rate')
    ...
    parser.add_argument('--adam_beta_min', type=float, default=-1, help=" adam_beta ")
    ...
    opt.model_name = 'S:{}_T:{}_{}_stage1_{}'.format(opt.model_s, opt.model_t, opt.dataset, opt.trial)
...
def main():
    ...
    model_s = model_dict['DiffModel'](model=model, classifier=classifier, feature_dim=opt.feature_dim, target_dim=opt.target_dim, stage='stage-1')
    ...
    # optimizer
    optimizer = torch.optim.AdamW(trainable_list.parameters(), lr=opt.learning_rate, betas=(0.9, 0.999), weight_decay=opt.weight_decay)
    ...
    if opt.resume is not None:
        ...
        model_s.load_state_dict(checkpoint, strict=True)
