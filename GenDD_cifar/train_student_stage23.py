...
from dataset.cifar100 import get_cifar100_dataloaders_sample, get_cifar100_dataloaders2
...
def main():
    ...
    if opt.dataset == 'cifar100':
        train_loader, val_loader, n_data = get_cifar100_dataloaders2(batch_size=opt.batch_size, num_workers=opt.num_workers, is_instance=True)
        n_cls = 100
    else:
    ...
    model_s = model_dict['DiffModel'](model=model, classifier=classifier, feature_dim=opt.feature_dim, target_dim=opt.target_dim, finetune_classifier=opt.finetune_classifier, stage='stage-2', smooth=opt.smooth)
    ...
    optimizer = torch.optim.AdamW(param_groups, betas=(0.9, 0.95), lr=opt.learning_rate)
