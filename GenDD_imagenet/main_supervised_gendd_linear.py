...
parser.add_argument('--alpha', type=float, default=1.0, help=" depressed hyper-parameter ")
...
parser.add_argument('--num_sampling', type=int, default=1, help=" sampling for representation aggretation ")
        ...
        if torch.cuda.is_available():
            if args.gpu is not None:
                ...
                if args.reload:
                    ...
                    checkpoint = torch.load(args.reload, map_location='cpu')['state_dict']
                    del checkpoint['module.model.fc.weight']
                    del checkpoint['module.model.fc.bias']
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            if len(target.shape) == 1:
                smooth_target = args.alpha * F.one_hot(target, num_classes=1000) + (1-args.alpha) * F.softmax(logits_t, dim=-1)
            else:
        ...
        loss_ce = criterion(output, smooth_target)
