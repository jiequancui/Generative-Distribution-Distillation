...
from models.diff_model_sample_mix import DiffModel
...
parser.add_argument('--adam_beta', type=float, default=0.95, help=" adam_beta ")
parser.add_argument('--diff_weight_decay', type=float, default=0., help=" weight decay for diff parameters ")
parser.add_argument('--diff_lr', type=float, default=0., help=" weight decay for diff parameters ")
...
def param_groups_setup(model, args, skip_list=()):
    ...
    return [
            {'params': diff_params, 'weight_decay': args.diff_weight_decay, 'lr': args.lr},
            {'params': non_diff_params, 'weight_decay': args.weight_decay, 'lr': args.diff_lr}]
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # student model
    model = DiffModel(model, model_t.fc, feature_dim=args.feature_dim_s, target_dim=args.feature_dim_t, stage='stage-2', smooth=args.smooth, beta=args.beta, diffloss_w=args.diffloss_w, diffusion_batch_mul=args.diffusion_batch_mul, finetune_classifier=args.finetune_classifier)
    ...
    if not args.finetune_classifier: 
        ...
        optimizer = torch.optim.AdamW(param_groups, betas=(0.9, 0.95), lr=args.lr)
    else:
