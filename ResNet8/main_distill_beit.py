...
parser.add_argument('--teacher_model', default=None, type=str, help=" path to teacher models")
...
def param_groups_setup(model, args, skip_list=()):
    ...
    return [
        {'params': diff_params, 'weight_decay': 0.},
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.teacher_arch.startswith('beit'):
    ...
    # student model
    model = DiffModel(model, model_t.head, feature_dim=args.feature_dim_s, target_dim=args.feature_dim_t, stage='stage-2', smooth=args.smooth, beta=args.beta, diffloss_w=args.diffloss_w, diffusion_batch_mul=args.diffusion_batch_mul)
    ...
    if args.evaluate:
        ...
        validate(train_loader, model_t, criterion, args)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # lr schedule
        lr=adjust_learning_rate_cos(epoch, optimizer, args)
...
def train(train_loader, model, model_t, criterion, optimizer, epoch, device, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            scores = (F.softmax(logits_t) * F.one_hot(target, num_classes=1000)).sum(dim=1)
        loss_diff, output = model(images, target, target_feature=feats_t[-1], weight=scores)
