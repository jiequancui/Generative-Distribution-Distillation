...
from models.diff_model_nomix import DiffModel
from models import resnet
from models import mobilenetv1

model_names = ['resnet18', 'resnet34', 'resnet50', 'MobileNetV1']
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # student model
    model = DiffModel(model, model_t.fc, feature_dim=args.feature_dim_s, target_dim=args.feature_dim_t, stage='stage-2', smooth=args.smooth, beta=args.beta, diffloss_w=args.diffloss_w, diffusion_batch_mul=args.diffusion_batch_mul, finetune_classifier=args.finetune_classifier, token_dim=args.token_dim, cond_drop_prob=args.cond_drop_prob)
...
def train(train_loader, model, model_t, criterion, optimizer, epoch, device, args):
    ...
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, diff_losses, top1, top5],
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            mask = torch.argmax(logits_t, dim=-1) == target
        loss_diff, output = model(images, target, target_feature=feats_t[-1], weight=mask)
