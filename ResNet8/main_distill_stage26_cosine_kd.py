...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, model_t, criterion, criterion_kl, optimizer, epoch, device, args)
...
def train(train_loader, model, model_t, criterion, criterion_kl, optimizer, epoch, device, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        output = model(images)
        loss = criterion_kl(F.log_softmax(output, dim=-1), target).sum(dim=-1).mean()
