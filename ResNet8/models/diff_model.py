...
class DiffModel(nn.Module):
    ...
    def forward_test_stage2(self, x):
        ...
        z = self.cls_projector(feats[-1]).unsqueeze(1).repeat(1, self.chunk_idx.size(1), 1)
        chunk_idx = self.chunk_idx.repeat(x.size(0), 1)
        z = z.reshape(-1, z.size(-1))
        chunk_idx = chunk_idx.reshape(-1,)
        ...
        # multiple sampling
        #for i in range(9):
        #    sample_features = self.diffloss.sample(z, chunk_idx, temperature=1.0, cfg=1.8)
        #    sample_features = sample_features.reshape(bsz*2, -1)
        #    logits += self.classifier(sample_features[:bsz])
        #logits = logits / 10.0
