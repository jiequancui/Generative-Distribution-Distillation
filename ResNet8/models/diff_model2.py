...
class DiffModel(nn.Module):
    def __init__(self, model, classifier=None, finetune_classifier=False, feature_dim=1024, target_dim=1024, diffloss_d=3, diffloss_w=1024,  num_sampling_steps='100', diffusion_batch_mul=50, grad_checkpointing=False, stage="stage-1", smooth=0.8, beta=1.0, token_dim=64):
    ...
    def forward_train_stage2(self, x, y, target_feature, weight=None):
        ...
        assert target_feature.size(-1) % self.token_dim == 0, "target feature dim should be divided by token dim"
        target_feature = target_feature.reshape(-1, target_feature.size(-1)//self.token_dim, self.token_dim)
        z = z.reshape(-1, z.size(-1)//self.token_dim, self.token_dim)
        bsz, seq_len, _ = target_feature.shape
        target_feature = target_feature.reshape(bsz * seq_len, -1).repeat(self.diffusion_batch_mul, 1)
    ...
    def forward_test_stage2(self, x):
        ...
        sample_features = self.diffloss.sample(z, temperature=1.0)
        sample_features = sample_features.reshape(bsz, -1)
        logits = self.classifier(sample_features)
        # multiple sampling
        #for i in range(9):
        #    sample_features = self.diffloss.sample(z, temperature=1.0)
        #    logits += self.classifier(sample_features.reshape(bsz,-1))
        #logits = logits / 10.0
