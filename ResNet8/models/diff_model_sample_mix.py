...
class DiffModel(nn.Module):
    ...
    def forward_train_stage2(self, x, y, target_feature, weight=None):
        ...
        lam = torch.distributions.Beta(self.beta, self.beta).sample([z.size(0)]).to(z)
        lam = lam.view(-1, 1)
        mix_z = z * lam + (1-lam) * z[indices]
        mix_target_feature = target_feature * lam + (1-lam) * target_feature[indices]
