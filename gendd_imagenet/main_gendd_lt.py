...
from dataset.imagenetlt import ImageNetLT
from utils import shot_acc

model_names = ['resnet18', 'resnet34', 'resnet50', 'MobileNetV1', 'beitv2_base_patch16_224', 'beitv2_large_patch16_224', 'resnext101_32x4d']
...
parser.add_argument('--crop_scale', type=float, default=0.6, help=" whether weak aug ")
...
parser.add_argument('--alpha', type=float, default=1.0, help=" depressed ")
parser.add_argument('--beta', type=float, default=1.0, help=" distribution contraction technique ")
...
parser.add_argument('--reload', type=str, help=" weight from pretraining ")
parser.add_argument('--smooth', type=float, default=0.9, help=" distribution contraction technique ")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.teacher_arch.startswith('res'):
        model_t = getattr(resnet, args.teacher_arch)(pretrained=False)
        checkpoint = torch.load(args.teacher_model)['state_dict']
        new_checkpoint = {}
        for key in checkpoint.keys():
            new_key = key[7:] #module.
            new_checkpoint[new_key] = checkpoint[key]
        model_t.load_state_dict(new_checkpoint, strict=True)
    ...
    # student model
    model = DiffModel(model, classifier, feature_dim=args.feature_dim_s, target_dim=args.feature_dim_t, smooth=args.smooth, num_sampling=args.num_sampling, stage='stage-2', beta=args.beta, diffloss_w=args.diffloss_w, diffusion_batch_mul=args.diffusion_batch_mul, token_dim=args.token_dim, cond_drop_prob=args.cond_drop_prob, finetune_classifier=args.finetune_classifier)
    ...
    if not args.finetune_classifier: 
        param_groups = param_groups_setup(model_without_ddp, args)
        optimizer = torch.optim.AdamW(param_groups, betas=(0.9, args.adam_beta_min), lr=args.lr)
    else:
    ...
    if args.dummy:
        ...
        txt_train = f'./imagenet_inat/data/ImageNet_LT/ImageNet_LT_train.txt'
        txt_test = f'./imagenet_inat/data/ImageNet_LT/ImageNet_LT_test.txt'
        train_dataset = ImageNetLT(root=args.data, txt=txt_train, transform=regular_transform)
    ...
    if args.evaluate:
        validate(val_loader, train_loader, model, criterion_val, args)
        return

    # evaluate teacher model performance before training
    validate(val_loader, train_loader, model_t, criterion_val, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, model_t, mixup_fn, criterion, optimizer, epoch, device, args)

        # evaluate on validation set
        acc1 = validate(val_loader, train_loader, model, criterion_val, args)
...
def train(train_loader, model, model_t, mixup_fn, criterion, optimizer, epoch, device, args):
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    diff_losses = AverageMeter('Diffloss', ':.4e')

    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, diff_losses],
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        with torch.no_grad():
            feats_t, logits_t = model_t(images, return_features=True)
            if len(target.shape) == 1:
                smooth_target = args.alpha * F.one_hot(target, num_classes=args.nb_classes) + (1-args.alpha) * F.softmax(logits_t, dim=-1)
            else:
                smooth_target = args.alpha * target + (1-args.alpha) * F.softmax(logits_t, dim=-1)

        loss_diff, output = model(images, smooth_target, target_feature=feats_t[-1])
        loss_ce = criterion(output, target)
        loss = loss_diff + loss_ce

        # measure accuracy and record loss
        losses.update(loss.item(), images.size(0))
        diff_losses.update(loss_diff.item(), images.size(0))

        # compute gradient and do SGD step
        optimizer.zero_grad()
        loss.backward()

        beta2 = beta2_schedule_cos(epoch * len(train_loader) + i, args.epochs * len(train_loader) - 1, args)
...
def validate(val_loader, train_loader, model, criterion, args):
    ...
    def run_validate(loader, base_progress=0):
        total_logits = torch.empty((0, args.nb_classes)).cuda()
        ...
        with torch.no_grad():
            ...
            for i, (images, target) in enumerate(loader):
                ...
                total_logits = torch.cat((total_logits, output_1))
        ...
        open(args.mark+"/log.txt","a+").write(' * Acc@1 {top1.avg:.3f} Acc@5 {top5.avg:.3f}\n'
        ...
        open(args.mark+"/log.txt","a+").write('Many_acc: %.5f, Medium_acc: %.5f Low_acc: %.5f\n'%(many_acc_top1, median_acc_top1, low_acc_top1))
