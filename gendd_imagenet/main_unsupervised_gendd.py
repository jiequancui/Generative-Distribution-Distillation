...
model_names = ['resnet18', 'resnet34', 'resnet50', 'MobileNetV1', 'beitv2_base_patch16_224']
...
parser.add_argument('--cond_drop_prob', type=float, default=0.2, help=" adam_beta ")
...
parser.add_argument('--smooth', type=float, help=" distribution contraction technique ")
...
# CC3M
parser.add_argument('--T', type=int, default=5500, help=" iterations each epoch ")
...
def beta2_schedule_linear(step, total_steps, args):
    beta2_start = args.adam_beta_min
    beta2_end = args.adam_beta_max
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dummy:
        ...
        all_shards = sorted(glob.glob("/mnt/proj205/jqcui/code/imagenet_cls/data/cc3m/cc3m-wds/cc3m-train-*.tar"))
        shards_for_rank = all_shards[rank::world_size]

        train_dataset = (
            wds.WebDataset(shards_for_rank)
            .shuffle(10000, rng=random.Random(0))
            .decode("pil")
            .to_tuple("jpg", "txt")
            .map_tuple(transform, preprocess_label)
            .batched(args.batch_size_per_gpu)  # Local batch size per GPU
        )
        train_loader = torch.utils.data.DataLoader(train_dataset, num_workers=args.workers, batch_size=None)
    ...
    validate(val_loader, model_t, criterion, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, model_t, criterion, optimizer, epoch, device, args)
...
def train(train_loader, model, model_t, criterion, optimizer, epoch, device, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        loss_diff, output = model(images, target, target_feature=feats_t[-1])
        loss = loss_diff
        ...
        beta2 = beta2_schedule_cos(epoch * T + i, args.epochs * T - 1, args)
