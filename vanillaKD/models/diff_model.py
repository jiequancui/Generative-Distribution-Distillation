...
class DiffModel(nn.Module):
    def __init__(self, model, classifier=None, finetune_classifier=False, feature_dim=1024, target_dim=1024, diffloss_d=3, diffloss_w=2048,  num_sampling_steps='64', diffusion_batch_mul=5, grad_checkpointing=False, stage="stage-1"):
    ...
    def forward_train_stage2(self, x, y, target_feature):
        ...
        with torch.no_grad():
            if len(y.shape) == 1:
                ...
                centers = y @ self.cls_centers #1000 x d  
            target_feature = target_feature * 0.2 + centers * 0.8
    ...
    def forward_test_stage2(self, x):
        ...
        #for i in range(9):
        #    sample_features = self.diffloss.sample(z, temperature=1.0)
        #    logits += self.model.fc(sample_features)
        #logits = logits / 10.0
