...
parser.add_argument('--dist-beta', type=float, default=1)
parser.add_argument('--dist-gamma', type=float, default=1)
...
parser.add_argument('--identical-init', action='store_true',
                    help='identical init params for all models (only for models of the same architecture)')
# ---------------------------------------------------------------------------------------
...
parser.add_argument('--no-ema-checkpoint', action='store_true', default=False,
                    help='Initialize model from no ema checkpoint (student only)')
...
def main():
    ...
    if args.identical_init:
        model = create_model(
            ...
            checkpoint_path='')  # load checkpoint later
        if args.initial_checkpoint:
            load_checkpoint(model, args.initial_checkpoint, use_ema=(not args.no_ema_checkpoint))
        model_list = [deepcopy(model) for _ in range(setting_num)]
        del model
    else:
        model_list = []
        for i in range(setting_num):
            model = create_model(
                ...
                bn_eps=args.bn_eps,
                scriptable=args.torchscript,
                checkpoint_path='')  # load checkpoint later TODO:

            args.feature_dim_t = teacher.head.weight.size(1)
            args.feature_dim_s = model.fc.weight.size(1)
            model = DiffModel(model, teacher.head, feature_dim=args.feature_dim_s, target_dim=args.feature_dim_t, stage='stage-2')

            if args.initial_checkpoint:
                load_checkpoint(model, args.initial_checkpoint, use_ema=(not args.no_ema_checkpoint), strict=False)
    ...
    # else:
    for i in range(setting_num):
        param_groups = [{"params": [p for n, p in model_list[i].named_parameters() if "ln" in n or "diffloss" in n], "weight_decay": 0.0,},
                        {"params": [p for n, p in model_list[i].named_parameters() if "ln" not in n and "diffloss" not in n], "weight_decay": setting_dicts[i]['weight_decay'],}]

        #optimizer_list.append(create_optimizer_v2(param_groups, opt=setting_dicts[i]['opt'], lr=setting_dicts[i]['lr'], momentum=args.momentum))
        optimizer_list.append(create_optimizer_v2(param_groups, opt=setting_dicts[i]['opt'], lr=setting_dicts[i]['lr'], betas=(0.9, 0.95)))
    ...
    for i in range(setting_num):
        ...
        if setting_dicts[i]['kd_loss'] == 'kd':
            kd_loss_fn = KLDiv(temperature=args.temperature).cuda()
        elif setting_dicts[i]['kd_loss'] == 'dist':
            kd_loss_fn = DIST(beta=args.dist_beta, gamma=args.dist_gamma, tau=args.temperature).cuda()
        elif setting_dicts[i]['kd_loss'] == 'bkd':
            kd_loss_fn = BinaryKLDiv().cuda()
        elif setting_dicts[i]['kd_loss'] == 'dkd':
            kd_loss_fn = DKD(alpha=args.dkd_alpha, beta=args.dkd_beta, temperature=args.temperature).cuda()
        else:
            raise NotImplementedError(f'unknown kd loss {args.kd_loss}')
        kd_loss_fn_list.append(kd_loss_fn)
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics_list = train_one_epoch(
                epoch, model_list, teacher, loader_train, optimizer_list, train_loss_fn_list, kd_loss_fn_list, args,
...
def train_one_epoch(
        epoch, model_list, teacher, loader, optimizer_list, loss_fn_list, kd_loss_fn_list, args,
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            with torch.no_grad():
                feats, _ = teacher(teacher_input, return_features=True)
        ...
        for i in range(setting_num):
            ...
            with amp_autocast():
                ...
                diffloss, output = model_list[i](student_input, targets[i], target_feature=feats[-1])
